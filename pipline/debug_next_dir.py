...
try:
    ...
    fonts = {k: ImageFont.load_default() for k in ['title', 'idx']}
...
for i in range(N_SAMPLES):
    ...
    # A tangent (average of prev->curr and curr->next)
    a_tangents = []
    if i > 0:
    ...
    # B tangent (average of prev->curr and curr->next)
    b_tangents = []
    if i > 0:
...
# Print debug info for problem points
